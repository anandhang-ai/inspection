...
async function main() {
    ...
    console.log("Inspecting BAPI_CUSTOMER_CREATEFROMDATA1...");
    const result = await client.callTool({
        name: "search_sap",
        arguments: {
            name: "Customer BAPIs",
            description: "BAPIs for Customer Create",
            apis: [{
                name: "Customer",
                description: "Customer Master Data",
                system_type: "SAP_ECC",
                functions: [
                    { name: "BAPI_CUSTOMER_CREATEFROMDATA1", description: "Create customer", rfc: true }
                ]
            }]
        }
    });
