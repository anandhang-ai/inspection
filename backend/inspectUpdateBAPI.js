...
async function main() {
    ...
    console.log("Inspecting BAPI_CUSTOMER_CHANGEFROMDATA1...");
    const result = await client.callTool({
        name: "search_sap",
        arguments: {
            name: "Customer BAPIs",
            description: "BAPIs for Customer Change",
            apis: [{
                name: "Customer",
                description: "Customer Master Data",
                system_type: "SAP_ECC",
                functions: [
                    { name: "BAPI_CUSTOMER_CHANGEFROMDATA1", description: "Change customer", rfc: true }
                ]
            }]
        }
    });
